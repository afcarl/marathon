...
trait AuthResource extends RestResource {
  import RestResource.{ RejectionException, AccessDeniedRejection, NotAuthenticatedRejection, ServiceUnavailableRejection }
  ...
  def authenticatedAsync(request: HttpServletRequest): Future[Identity] = {
    ...
    authenticationRequest.transform {
      case Success(Some(identity)) =>
        Success(identity)
      case Success(None) =>
        Failure(RejectionException(NotAuthenticatedRejection(authenticator, request)))
      case Failure(e) =>
        Failure(RejectionException(ServiceUnavailableRejection))
    }
  }
  ...
  def checkAuthorization[A, B >: A](action: AuthorizedAction[B], resource: A)(implicit identity: Identity): A = {
    ...
    else throw RestResource.RejectionException(AccessDeniedRejection(authorizer, identity))
  }
